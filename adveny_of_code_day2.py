        ...
        for part in parts[1].split(';'):
            for item in part.split(','):
                ...
                if item.endswith('red'):
                    sum_of_red= max(sum_of_red, count)
                elif item.endswith('blue'):
                    sum_of_blue= max(sum_of_blue, count)
                elif item.endswith('green'):
                    sum_of_green= max(sum_of_green, count)
        power_sum = sum_of_red*sum_of_blue*sum_of_green
        total_sum+=power_sum
                
                # if item.endswith('red') and int(item.split()[0])>12:
                #     possible=False
                # elif item.endswith('blue') and int(item.split()[0])>14:
                #     possible=False
                # elif item.endswith('green') and int(item.split()[0])>13:
                #     possible=False
        # if possible:
        #     total_sum+=game_id
    return total_sum
